...
class GraphColorizer(nn.Module):
    def __init__(self, graph: Graph = None, device="cpu", loss_type="reinforce"):
        super().__init__()
        self.embedding_size = 128 # 512
        self.n_possible_colors = 128 # 256
    ...
    def forward(self, graph: Graph, baseline=None):
        ...
        # The first layer is done separately, cuz the varying dimension means that it cannot have skip connections:
        neighbor_updates = self.neighb_attns[0].forward(embeddings, adj_matrix)
        non_neighbor_updates = self.non_neighb_attns[0].forward(embeddings, inverted_adj_matrix)
        concatenated = torch.cat((neighbor_updates, non_neighbor_updates), dim=1)
        embeddings = self.attn_combiner[0].forward(concatenated)

        for i in range(1, self.n_attn_layers):
