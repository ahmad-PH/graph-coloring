...
def test_on_single_graph():
    ...
    confidences = []
    log_confidences = []
    data.n_used_lambda_scheduler = LinearScheduler(0.01, 0.1, n_epochs)
    ...
    for i in range(n_epochs):
        ...
        optimizer.zero_grad()
        with torch.no_grad():
            _, _, g_extra = colorizer.forward(graph, embeddings, None, greedy=True)
        # TODO: run the code and see the bug that happens. It's cuz you're calling twice
        # maybe use two schedulers (better), or give the scheduler a "get_value" method.
        # Afterwards, you need to carefully examine the parts you've changed according to git
        # and make sure no bugs are introduced.
        baseline = g_extra.cost

        coloring, loss, extra = colorizer.forward(graph, embeddings, baseline)

        n_used_colors = len(set(coloring))
        print('props', coloring_properties(coloring, graph))
        print('n_used:', n_used_colors)
        print('cost: {}, new baseline: {}'.format(extra.cost, baseline))
